...
def A_pay(s, n, i):
    percent = i / 100
    k = percent * (1 + percent) ** n / (((1 + percent) ** n) - 1)
    a_pay = round(k * s, 2)
    return a_pay


def periods(credit, percent, a_pay, period):
    percent = percent / 100
    for periods in range(1, period + 1):
        pay_percent = percent * credit
        paid_credit = a_pay - pay_percent
        print('\nПериод: ', periods)
        print('Остаток на начало периода:', credit)
        print('Выплачено процентов: ', pay_percent)
        print('Выплачено тело: ', paid_credit)
        credit -= paid_credit
    else:
...
a_pay = A_pay(credit, years, percent)
new_summ = periods(credit, percent, a_pay, 3)
print('\n=================================================')

new_years = int(input('\nНа сколько лет продляется договор: '))
new_percent = float(input('Увеличние ставки до: '))
new_years = new_years + years - 3

a_pay = A_pay(new_summ, new_years, new_percent)
new_summ = periods(new_summ, new_percent, a_pay, new_years)
